...
import frc.robot.commands.ColorArmCommand;
import frc.robot.commands.ColorWheelFindCommand;
...
public class RobotContainer {
  ...
  public static ColorWheelSubsystem m_colorWheelSubsystem = new ColorWheelSubsystem();
  public static ColorArmSubsystem m_colorArmSubsystem = new ColorArmSubsystem();
  ...
  public RobotContainer() {
    ...
    climbSubsystem.setDefaultCommand(new ClimbCommand(climbSubsystem, shooterJoystick));
  }
  ...
  private void configureButtonBindings() {
    ...
    wheelSpinButton.whenPressed(new ColorWheelSpinCommand(m_colorWheelSubsystem, m_colorSensor));
    wheelStopButton.whenPressed(new ColorWheelStopCommand(m_colorWheelSubsystem, m_colorSensor));
   // wheelFindButton.whenPressed(new ColorWheelFindCommand(m_colorWheelSubsystem, m_colorSensor));
    wheelUpButton.whenPressed(new ColorArmCommand(m_colorArmSubsystem, "Out"));
    wheelDownButton.whenPressed(new ColorArmCommand(m_colorArmSubsystem, "In"));
    CommandBase openClimb = new StartEndCommand(climbSubsystem::unlockClimber,climbSubsystem::stopUnlockClimber,climbSubsystem).withTimeout(0.5);
    openClimbButton.whenPressed(openClimb);
...
System.out.println("in autonomous");
    return new SequentialCommandGroup(
     ...
     // Aim at the target
     // new AimCommand(driveSubsystem, m_indexSubsystem, m_shooterSubsystem).withTimeout(3),
      new WaitCommand(2),
      new AimCommand(driveSubsystem, m_indexSubsystem, m_shooterSubsystem),
      new AimCommand(driveSubsystem, m_indexSubsystem, m_shooterSubsystem),
      
      //Start Shooter
      new ShootByDistanceCommand(m_shooterSubsystem), 
      new WaitCommand(2),
      new ElevatorCommand(m_elevatorSubsystem, "On"),

      //Shoot all 3 balls
      new IndexCommand(m_indexSubsystem, m_shooterSubsystem).withTimeout(5),

      //Start Elevator


      //new WaitCommand(3), //wait for motor to speed up
      //new AimCommand(driveSubsystem, m_indexSubsystem, m_shooterSubsystem),

      // Shoot the second ball
      //new IndexCommand(m_indexSubsystem, m_shooterSubsystem).withTimeout(1),
      //new WaitCommand(2),
      ...
      new ElevatorCommand(m_elevatorSubsystem, "Off"),

      //turn off shooter
      new ShootCommand(m_shooterSubsystem, 0),
